  ...
  class UISchemaFactory {
    ...
    updateFieldsBasedOnConditions(formData, setJsonSchema) {
      if (this.formData?.account_info === "CORPORATE") {
        this.setExternalUrl(
          "/kyc-update/update-requests/corporate-kyc-update-form"
        );
      } else {
        this.setExternalUrl(
          "/kyc-update/update-requests/individual-kyc-update-form"
        );
      }
      setJsonSchema((prevSchema) => {
        if (!prevSchema || !prevSchema.properties) {
            ...
            onChange: (value) => {
              const changedValue =
                value === "CORPORATE"
                  ? "/kyc-update/update-requests/corporate-kyc-update-form"
                  : value === "MINOR"
                  ? "/kyc-update/update-requests/minor-kyc-update-form"
                  : "/kyc-update/update-requests/individual-kyc-update-form";
