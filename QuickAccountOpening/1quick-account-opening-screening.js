    ...
    async updateFormAndSchema(formData, schemaConditions) {
      if (this.formData?.current_step === "case-review") {
        ...
        [
          ...
          "coo",
          "Completed",
        ].includes(this.formData?.current_step)
      ) {
        this.setJsonSchema((prevSchema) => {
          if (!prevSchema) return prevSchema;
          return {
        ...
        this.setNextStep("quick-account-opening-full-form");
        this.setDivide(true);
      }
    ...
    createUISchema(options) {
      ...
      return {
          ...
          "ui:options": {
            getOptions: (formData, index) => {
              return this.optionsData["account_category"]
                ?.filter((data) => ["Individual"].includes(data?.title))
                ?.map((item) => ({
                  label: item.title,
                  value: item?.fg_code || item?.cbs_code || item?.id,
                }));
        ...
        approval_status: {
          ...
          "ui:widget": this.formData?.current_step?.includes("case-review")
            ? "SelectWidget"
            : "hidden",
        },
