        ...
        Object.entries(data).forEach(([key, value]) => {
          ...
          if (isDocuments && Array.isArray(value)) {
            ...
            value.forEach((doc, index) => {
              const fieldKey = doc?.cbs_code || `doc_${index}`;
              if (doc?.is_required) {
              ...
              formData.document.details[fieldKey] = "";
            });
            ...
            value.forEach((item, idx) => {
              ...
              if (item?.guardian && item?.guardian?.documents?.length > 0) {
                const itemKey =
                  item?.guardian?.name?.replace(/\s+/g, "_").toLowerCase() ||
                  `item_name${idx}`;
                ...
                if (item?.guardian.name) {
                  itemSchema.properties.name = {
                    type: "string",
                    title: "Name",
                    default: item?.guardian.name,
                    readOnly: true,
                  };
                  itemFormData.name = item?.guardian.name;
                }

                if (Array.isArray(item?.guardian.documents)) {
                  const docFields = {};
                  const docUi = {};
                  const docRequired = [];
                  const docFormData = {};

                  item?.guardian.documents.forEach((doc, docIndex) => {
                    const fieldKey = doc?.cbs_code || `doc_${docIndex}`;
                    if (doc?.is_required) {
    ...
    async initializeSchema(setJsonSchema, formData) {
      ...
      try {
        const response = await this.axios.post(
          `${this.mainRouteURL}/get-document/corporate`,
          {
