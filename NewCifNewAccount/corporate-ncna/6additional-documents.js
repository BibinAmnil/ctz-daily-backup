(function () {
  ...
  class UISchemaFactory {
    ...
    async updateSchemaWithEnums(
      ...
      const fieldMapping = { account_operation_signature: "account_operation" };
      ...
      setJsonSchema((prevSchema) => {
        ...
        const updateProperties = (schema) => {
          ...
          // if (schema.if) {
          //   if (schema.then) updateProperties(schema.then);
          //   if (schema.else) updateProperties(schema.else);
          // }
        };
    ...
    async initializeSchema(setJsonSchema, formData) {
      const fieldsToUpdate = ["account_operation_signature"];
